 ...
 * */
class FudgeTvVideoPlayerState internal constructor(
    @IntRange(from = 0L)
...
@Composable
fun rememberVideoPlayerState(@IntRange(from = 0L) hideSeconds: Int = 4): FudgeTvVideoPlayerState =
    remember { FudgeTvVideoPlayerState(hideSeconds = hideSeconds) }
        .also {
            LaunchedEffect(it) {
                it.observe()
            }
